...
const switcher=(language,ruPath,enPath)=>{const current=language==='en'?'EN':'RU';return `<div class="language-switcher" data-language-switcher><button class="language-switcher__trigger" type="button" aria-label="${language==='en'?'Choose language':'Выбрать язык'}" aria-haspopup="true" aria-expanded="false">${current}</button><div class="language-switcher__menu" role="menu"><a href="${ruPath}" data-language="ru" lang="ru" role="menuitem"${language==='ru'?' aria-current="true"':''}>RU — ${language==='en'?'Russian':'Русский'}${language==='ru'?' <span aria-hidden="true">✓</span>':''}</a><a href="${enPath}" data-language="en" lang="en" role="menuitem"${language==='en'?' aria-current="true"':''}>EN — English${language==='en'?' <span aria-hidden="true">✓</span>':''}</a></div></div>`};
